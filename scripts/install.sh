...
# Instalador do Haval Impulse (FORK leandrosavn) — versao adaptada do script original do bobaoapae.
# Roda na central via telnet root (porta 23), de preferencia logo apos um factory reset (UID baixo).
# Unica mudanca vs o original: o APK do Haval vem do fork leandrosavn/haval-impulse (release .64 com temas).
...
# Script principal
main() {
    log "INFO" "Iniciando instalacao (fork leandrosavn)..."
    ...
    download "https://github.com/leandrosavn/haval-impulse/releases/download/v1.0.0.64-preview/app-release.apk" "haval.apk" "Haval App (fork .64 com temas)"
    ...
    # Injecao (hook no system_server que fura o bloqueio de instalacao da Beantechs)
    ...
    install_app "haval.apk" "Haval Impulse (fork)"

    # Limpa arquivos temporarios
    rm -f shizuku.apk haval.apk
    ROLLBACK_ENABLED=false

    echo "Instalacao concluida! Abra o Impulse, conceda o Shizuku, e pronto."
}
